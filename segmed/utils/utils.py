...
from typing import Optional, Tuple, Generator, Iterator, Union


def extract_data(
    train_path: str, label_path: Optional[str] = None, rgb: Optional[bool] = False
    ...
    Returns:
        X_train, y_train: Arrays of the images and their segmentation maps, normalized.
        X_train: Array of the images and their segmentation maps, normalized; when there
            is no `label_path`.
    """
...
def split_images(
    ...
    Args:
        x: Array of images with shape
            (number_of_images, height, width).
        y: Array of images with shape
            (number_of_images, height, width).
        size: (height, width) of the resulting patches.
        num_part: Integer value; the number of resulting patches.

    Returns:
        x_imgs, y_imgs: The cropped patches from the original images.
        x_imgs: The cropped patches from the original images, if only `x` is passed.
    """
    x_patches = image.PatchExtractor(
        patch_size=size, max_patches=num_part, random_state=0
    )
    ...
    if not y is None:
        y_patches = image.PatchExtractor(
            patch_size=size, max_patches=num_part, random_state=0
        )
